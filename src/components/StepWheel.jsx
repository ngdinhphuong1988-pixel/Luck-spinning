    ...
    <div style={{ display:'flex', flexDirection:'column', alignItems:'center', width:'100%', padding:'0 16px' }}>
      ...
      <div style={{ display:'flex', gap:12, marginTop:18, alignItems:'center' }}>
        <button onClick={handleSpin} disabled={spinning} style={{
          ...
          boxShadow: spinning ? 'none' : '0 8px 38px rgba(255,100,50,0.4),0 0 0 8px rgba(255,107,107,0.1)',
        ...
        <button onClick={() => setShowBill(true)} style={{
          ...
          cursor:'pointer', boxShadow:'0 4px 20px rgba(107,203,119,0.2)',
        }}>
          <span style={{ fontSize:24 }}>💰</span>
          <span style={{ fontSize:9, fontWeight:900, color:'#6BCB77', letterSpacing:.5 }}>CHIA TIỀN</span>
        </button>
      </div>
      ...
      {winner && (
        <div style={{
          marginTop:18, padding:'16px 26px',
          ...
          <div style={{
            fontSize:24, fontWeight:900, color:'#FFD93D',
            fontFamily:"'Orbitron', sans-serif",
            textShadow:'0 0 16px rgba(255,217,61,.6)', wordBreak:'break-word',
          }}>{winner}</div>
      ...
      {/* Player chips */}
      <div style={{ marginTop:16, display:'flex', flexWrap:'wrap', gap:7, justifyContent:'center', maxWidth:310, padding:'0 8px' }}>
        {names.map((n, i) => (
          <span key={i} style={{
            padding:'4px 12px',
